...
def get_valid_applicant_by_token(db: Session, token: str) -> Applicant:
    ...
    # The employment form is generated by HR while the applicant is still
    # "reviewed" (see generate_employment_form in app/api/admin/applicants.py,
    # which only allows generation at that exact status). If "reviewed" isn't
    # allowed here too, the applicant would be locked out of the very link
    # HR just generated for them, until someone separately advances their
    # status to "interview" -- so it must stay in this list.
    if applicant.status not in ["reviewed", "interview", "onboarding", "hired"]:
        raise HTTPException(
            status_code=400,
